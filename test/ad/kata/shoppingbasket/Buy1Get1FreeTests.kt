...
class Buy1Get1FreeTests {
    ...
    fun `returns discount of 1 product price if item's quantity is 2`() {
        val items = listOf(PricedItem(Sku("1"), skuPrice = Euros(1.0), Quantity(2)))
    ...
    fun `returns discount of 1 product price if item's quantity is 3`() {
        val items = listOf(PricedItem(Sku("1"), skuPrice = Euros(1.0), Quantity(3)))
    ...
    fun `returns discount of 2 products price if item's quantity is 4`() {
        val items = listOf(PricedItem(Sku("1"), skuPrice = Euros(1.0), Quantity(4)))

        expectThat(
            Buy1Get1Free(Sku("1"))
        ) {
            get { discountFor(items) }.isEqualTo(
                Discount(Sku("1"), discountInEuros = Euros(2.0))
            )
        }
    }
    ...
    @Test
    fun `aggregates quantities of all items (assuming price is the same) to give discount`() {
